//
//  MessageAudioController.swift
//  ZaloChatApp
//
//  Created by huy on 28/11/2022.
//
...
// MARK: - MessageAudioController

/// The `MessageAudioController` update UI for current audio cell that is playing a sound
/// and also creates and manage an `AVAudioPlayer` states, play, pause and stop.
open class MessageAudioController: NSObject, AVAudioPlayerDelegate {
    // MARK: Lifecycle
    ...
    open func playSound(for message: MessageType, in audioCell: AudioMessageCell) {
        ...
        case .audio(let item):
            ...
            let destinationUrl = documentsDirectoryURL.appendingPathComponent(item.url.lastPathComponent)

            guard let player = try? AVAudioPlayer(contentsOf: destinationUrl) else {
                print("Failed to create audio player for URL: \(destinationUrl)")
                return
            }
            audioPlayer = player
        ...
        default:
            print("MessageAudioPlayer failed play sound because given message kind is not Audio")
        }
    }
    ...
    private func startProgressTimer() {
        ...
        progressTimer = Timer.scheduledTimer(
            timeInterval: 0.1,
            target: self,
            selector: #selector(MessageAudioController.didFireProgressTimer(_:)),
            userInfo: nil,
            repeats: true)
    }
}
